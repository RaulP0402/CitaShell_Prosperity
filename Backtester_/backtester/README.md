...
## Additional Bugs
It is possible that the changes made for compatibility introduced further bugs. It is noticeable that the PnL in the backtester is sometimes lower, sometimes higher than the PnL in the uploaded files. The reason for this could be that although the backtester works fine, the backtester has significantly less action in it since the trader is only able to trade against the orderbook, whereas in the simulation the trader can send orders that bots then choose to interact with.
...
* When calculating PnL via `trades_position_pnl_run`.
