...
# from MasterBacktesterLogger import Trader
# 2024 Prices Data: goes up to Timestamp 199900
...
TRAINING_DATA_PREFIX = 'test-data'
...
zero_round = ['AMETHYSTS', 'STARFRUIT']
...
zero_round_pst = ['AMETHYSTS', 'STARFRUIT']
...
def simulate_alternative(
    ...
    df_trades = pd.read_csv(trades_path, dtype={'seller': str, 'buyer': str})
    ...
    states, trader, profits_by_symbol, balance_by_symbol = trades_position_pnl_run(states, max_time, profits_by_symbol,
                                                                                   balance_by_symbol, credit_by_symbol,
                                                                                   unrealized_by_symbol)
    create_log_file(round, day, states, profits_by_symbol, balance_by_symbol, trader)
    profit_balance_monkeys = {}
    trades_monkeys = {}
    if monkeys:
        profit_balance_monkeys, trades_monkeys, profit_monkeys, balance_monkeys, monkey_positions_by_timestamp = monkey_positions(
            monkey_names, states, round)
    ...
    for time, state in states.items():
        ...
        if states.get(time + FLEX_TIME_DELTA) != None:
            ...
            for psymbol in SYMBOLS_BY_ROUND_POSITIONABLE[round]:
                ...
                if position[psymbol] == 0 and states[time].position[psymbol] != 0:
                    profits_by_symbol[time + FLEX_TIME_DELTA][psymbol] += credit_by_symbol[time + FLEX_TIME_DELTA][
                        psymbol]  # +unrealized_by_symbol[time + FLEX_TIME_DELTA][psymbol]
                    credit_by_symbol[time + FLEX_TIME_DELTA][psymbol] = 0
                    balance_by_symbol[time + FLEX_TIME_DELTA][psymbol] = 0
                else:
                    balance_by_symbol[time + FLEX_TIME_DELTA][psymbol] = credit_by_symbol[time + FLEX_TIME_DELTA][
                                                                             psymbol] + \
        ...
        if time == max_time:
            ...
            for osymbol in position.keys():
                profits_by_symbol[time + FLEX_TIME_DELTA][osymbol] += credit_by_symbol[time + FLEX_TIME_DELTA][
                                                                          osymbol] + \
                                                                      unrealized_by_symbol[time + FLEX_TIME_DELTA][
                                                                          osymbol]
...
def monkey_positions(monkey_names: list[str], states: dict[int, TradingState], round):
    ...
    trades_by_round: dict[int, dict[str, list[Trade]]] = {
        0: dict(zip(monkey_names, [[] for x in range(len(monkey_names))]))}
    ...
    for monkey in monkey_names:
        ...
        monkey_positions[monkey] = dict(
            zip(SYMBOLS_BY_ROUND_POSITIONABLE[round], [0] * len(SYMBOLS_BY_ROUND_POSITIONABLE[round])))
    ...
    for time, state in states.items():
        ...
        for monkey in monkey_names:
                    ...
                    for market_trade in state.market_trades[psymbol]:
                        if trades_by_round[time].get(market_trade.buyer) != None:
                            trades_by_round[time][market_trade.buyer].append(
                                Trade(psymbol, market_trade.price, market_trade.quantity))
            ...
            if states.get(time + FLEX_TIME_DELTA) != None:
                for psymbol in SYMBOLS_BY_ROUND_POSITIONABLE[round]:
                    ...
                    if position[psymbol] == 0 and prev_monkey_positions[monkey][psymbol] != 0:
                        profits_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol] += \
                        credit_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol]
                        credit_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol] = 0
                        balance_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol] = 0
                    else:
                        balance_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol] = \
                        credit_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol] + \
                        unrealized_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol]
                    profit_balance[time + FLEX_TIME_DELTA][monkey][psymbol] = \
                    profits_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol] + \
                    balance_by_symbol[time + FLEX_TIME_DELTA][monkey][psymbol]
            prev_monkey_positions[monkey] = copy.deepcopy(monkey_positions[monkey])
            ...
            if time == max_time:
                # i have the feeling this already has been done, and only repeats the same values as before
                for osymbol in position.keys():
                    profits_by_symbol[time + FLEX_TIME_DELTA][monkey][osymbol] += \
                    credit_by_symbol[time + FLEX_TIME_DELTA][monkey][osymbol] + \
                    unrealized_by_symbol[time + FLEX_TIME_DELTA][monkey][osymbol]
...
if __name__ == "__main__":
    trader = Trader()
    max_time = int(input("Max timestamp (1-9)->(1-9)(00_000) or exact number): ") or 199900)
    if max_time < 10:
        max_time *= 100000
    round = int(input("Input a round (blank for 4): ") or 0)
    day = int(input("Input a day (blank for random): ") or -2)
