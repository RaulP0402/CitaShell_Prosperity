...
class AbstractIntervalTrader:
    ...
    def buy(self, price, vol, prod = ""):
        ...
        vol = min(vol, self.limit - self.position - self.buy)
        if vol > 0:
    ...
    def sell(self, price, vol, prod = ""):
        ...
        vol =  max(-vol, -self.position - self.limit - self.sell)
        if vol < 0:
...
class Trader:
    def run(self, state: TradingState):
        from OLD_STUFF.abstract_arima import ARIMAModel
        return run_traders({
                            "STARFRUIT": ARIMAModel(20)
                            }, state)
# prosperity2bt abstract_tester.py 1